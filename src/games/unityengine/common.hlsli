...
float3 GradeAndDisplayMap(float3 color) {
  ...
  if (config.type == 0.f) {
    ...
    color = CorrectHueAndChrominanceOKLAB(color, renodx::tonemap::ReinhardPiecewise(color, injectedData.toneMapSDRClip, 0.99f), injectedData.toneMapSDRify, injectedData.toneMapSDRify);
    }
    color = HDRBoost(color, injectedData.fxHdrBoost);
  }
