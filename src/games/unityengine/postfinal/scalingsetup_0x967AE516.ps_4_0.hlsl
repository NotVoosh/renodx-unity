  ...
  out float4 o0 : SV_Target0)
{
  ...
  r6.xyzw = t0.SampleBias(s0_s, r3.zw, cb0[19].x).xyzw;
  r3.xyzw = t0.SampleBias(s0_s, r3.xy, cb0[19].x).xyzw;
  ...
  r2.z = renodx::color::y::from::BT709(saturate(r1.yzw));
  r2.w = min(r1.x, r2.y);
  r1.x = max(r1.x, r2.y);
  r1.x = max(r1.x, r0.x);
  r0.x = min(r2.w, r0.x);
  r3.xyzw = t0.SampleBias(s1_s, v1.xy, cb0[19].x).xyzw;
  r2.y = renodx::color::y::from::BT709(saturate(r3.xyz));
  r2.w = min(r2.y, r2.x);
  r2.x = max(r2.y, r2.x);
  r1.x = max(r2.x, r1.x);
  r0.x = min(r2.w, r0.x);
  o0.xyz = (r2.z > r1.x) || (r2.z < r0.x) ? r0.yzw : r1.yzw;
  o0.w = 1;
  return;
}