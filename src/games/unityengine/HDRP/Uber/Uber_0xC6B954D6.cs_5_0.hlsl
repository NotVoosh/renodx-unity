...
void main(uint3 vThreadID: SV_DispatchThreadID) {
  ...
  r2.xy = -cb0[51].zw * float2(0.5,0.5) + float2(1,1);
  r2.xy = min(r2.xy, r1.zw);
  r0.xy = cb0[54].xy * r2.xy;
  r2.xyzw = t0.SampleLevel(s0_s, r0.xyz, 0).xyzw;
  if (cb1[7].z != 0) {
    r3.xy = cb0[54].xy * r1.zw;
  ...
  if ((uint)cb1[1].z == 0) {
    r1.xy = r1.xy * cb0[51].zw + -cb1[1].xy;
    ...
    r3.xyz = cb1[3].www * r1.xyz + r0.xyz;
  }
  if (cb1[12].x != 0) {
    r3.xyz = lutShaper(r3.xyz);
  } else {
    if (cb1[6].w != 0) {
      ...
      r3.xyz = t3.SampleLevel(s1_s, r0.xyz, 0).xyz;
      } else {
        r3.xyz = renodx::lut::SampleTetrahedral(t3, r0.xyz, 1 / cb1[6].x);
      }
    }
  }
